...
export function MobileNav() {
  ...
  return <nav className="fixed inset-x-0 bottom-0 z-40 flex h-[74px] items-center justify-around border-t border-white/10 bg-[#0a171e]/95 px-3 backdrop-blur-xl lg:hidden">
    ...
    <a href="/settings" className={`flex flex-col items-center gap-1 ${tone(["/water","/livestock","/plants","/equipment","/calculators","/settings"].some(p=>pathname.startsWith(p)))}`}><Menu size={20}/><span className="text-[9px] font-bold">Daha fazla</span></a>
  </nav>;
}
