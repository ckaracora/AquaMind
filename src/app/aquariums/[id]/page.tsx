...
const tabs = [["Genel Bakış", ""], ["Sağlık Analizi", "health"], ["Su Değerleri", "/water"], ["Bakım", "/maintenance"], ["Canlılar", "/livestock"], ["Bitkiler", "/plants"], ["Ekipmanlar", "/equipment"]] as const;

export default function AquariumDetailPage() {
  const { id } = useParams<{ id: string }>();
  const { aquariums, hydrated, waterReadings, maintenanceTasks, livestock, plants, equipment } = useAquariums();
  ...
  return <AppShell><PageHeader title={aquarium.name}/><div className="mx-auto max-w-[1280px] px-5 py-7 sm:px-8 lg:px-10 lg:py-10">
    ...
    <nav className="mb-5 flex gap-1 overflow-x-auto rounded-xl border border-white/[.06] bg-[#0d1b23]/80 p-1.5">{tabs.map(([tab,path], index) => <a key={tab} href={path === "health" ? `/aquariums/${id}/health` : path ? `${path}?aquarium=${id}` : "#"} className={`shrink-0 rounded-lg px-4 py-2.5 text-[11px] font-bold ${index === 0 ? "bg-aqua/10 text-aqua" : "text-[#71858d] hover:text-white"}`}>{tab}</a>)}</nav>
    ...
    <div className="grid gap-5 xl:grid-cols-[1.4fr_.85fr]">
      ...
      <aside className="space-y-5"><section className="surface p-5 sm:p-6"><div className="mb-5 flex items-center justify-between"><div><p className="eyebrow">Bakım planı</p><h2 className="mt-1 text-lg font-extrabold">Yaklaşan görevler</h2></div><button className="grid size-9 place-items-center rounded-xl bg-white/[.04] text-aqua"><Plus size={16}/></button></div>{tasks.length ? <div className="space-y-2">{tasks.map((task, index) => <div key={task.id} className="flex items-center gap-3 rounded-xl border border-white/[.05] p-3.5"><span className={`grid size-9 shrink-0 place-items-center rounded-lg ${index === 0 ? "bg-aqua/10 text-aqua" : "bg-white/[.04] text-[#71858d]"}`}><CalendarDays size={16}/></span><div className="min-w-0 flex-1"><p className="truncate text-xs font-bold">{task.title}</p><p className="mt-1 text-[9px] text-[#647981]">{formatDate(task.dueAt)}</p></div></div>)}</div> : <EmptyLine text="Yaklaşan bakım görevi yok."/>}<button className="mt-4 w-full rounded-xl border border-white/[.06] py-3 text-xs font-bold text-[#82969e]">Bakım planını aç</button></section>
        <section className="surface p-5 sm:p-6"><p className="eyebrow">Hızlı işlem</p><div className="mt-4 grid grid-cols-2 gap-2"><QuickAction icon={Droplets} label="Ölçüm ekle" href={`/water?aquarium=${id}`}/><QuickAction icon={CalendarDays} label="Bakım ekle" href={`/maintenance?aquarium=${id}`}/><QuickAction icon={Fish} label="Canlı ekle" href={`/livestock?aquarium=${id}`}/><QuickAction icon={Leaf} label="Bitki ekle" href={`/plants?aquarium=${id}`}/></div></section>
      </aside>
    </div>
  </div></AppShell>;
}
...
function QuickAction({ icon: Icon, label, href }: { icon: typeof Fish; label: string; href: string }) { return <a href={href} className="flex flex-col items-start rounded-xl border border-white/[.06] bg-white/[.02] p-3.5 text-left text-[10px] font-bold text-[#91a3aa] hover:border-aqua/20 hover:text-aqua"><Icon size={16} className="mb-3"/>{label}</a>; }
