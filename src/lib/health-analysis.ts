...
export function analyzeAquarium(aquarium:Aquarium,animals:Livestock[],equipment:Equipment[],latest?:WaterParameters):HealthAnalysis{
 ...
 const filters=equipment.map(profileForEquipment).filter(p=>p?.category==="filter"); const ratedFlow=filters.reduce((s,p)=>s+(p?.ratedFlowLph??0),0); const turnover=ratedFlow*.65/Math.max(1,aquarium.netVolumeLiters);
 ...
 const filterScore=!filters.length?35:turnover<targetMin?clamp(70-(targetMin-turnover)*15):turnover>targetMax?clamp(75-(turnover-targetMax)*8):95;
 ...
 const metrics:HealthMetric[]=[
  ...
  {key:"filter",label:"Filtrasyon uygunluğu",score:filterScore,status:status(filterScore),detail:filters.length?`Tahmini ${turnover.toFixed(1)} çevrim/saat`:"Katalogdan filtre bulunamadı"},
 ...
 if(filters.length&&turnover<targetMin)warnings.push({level:"danger",title:"Filtrasyon sınırda",message:"Tahmini filtre çevrimi mevcut canlı yükü için düşük görünüyor."});
