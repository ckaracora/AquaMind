import { isVerifiedEquipmentProfile, isVerifiedSpeciesProfile, profileForEquipment, speciesForLivestock, speciesWaterTypes } from "@/data/catalog";
...
export function analyzeAquarium(aquarium:Aquarium,animals:Livestock[],equipment:Equipment[],latest?:WaterParameters):HealthAnalysis{
 ...
 const verifiedMatchedProfiles=matchedProfiles.filter(x=>isVerifiedSpeciesProfile(x.profile));
 ...
 const habitatIssues=profiles.filter(({profile})=>!speciesWaterTypes(profile!).includes(aquarium.type));
 ...
 const safetyEquipmentInputCount=equipment.filter(item=>{const profile=profileForEquipment(item);return !profile?.passiveComponent&&(item.category==="filter"||item.category==="heater"||(airDrivenFilters.length>0&&item.category==="air_pump"));}).length;
