...
export function analyzeAquarium(aquarium:Aquarium,animals:Livestock[],equipment:Equipment[],latest?:WaterParameters):HealthAnalysis{
 ...
 const compatibilityPenalty=(temperatureConflict?40:0)+(phConflict?35:0)+(flowConflict?20:0)+(predationIssues.length?60:0); const compatibilityScore=clamp(100-compatibilityPenalty);
 const matchedEquipment=equipment.map(profileForEquipment).filter(profile=>profile!==undefined); const verifiedEquipment=matchedEquipment.filter(isVerifiedEquipmentProfile);
 ...
 const safetyEquipment=matchedEquipment.filter(p=>p.category==="filter"||p.category==="heater"||Boolean(p.integratedHeaterW));
 const calculationReadyEquipment=safetyEquipment.filter(p=>{
  if(!isVerifiedEquipmentProfile(p))return false;
  const filterReady=p.category!=="filter"||Boolean(p.ratedFlowLph);
  const heaterReady=!(p.category==="heater"||p.integratedHeaterW)||Boolean(p.recommendedMinL||p.recommendedMaxL);
  return filterReady&&heaterReady;
 });
 const safetyEquipmentInputCount=equipment.filter(item=>item.category==="filter"||item.category==="heater").length;
 const totalDataCount=animals.length+safetyEquipmentInputCount; const verifiedDataCount=profiles.length+calculationReadyEquipment.length; const confidenceScore=totalDataCount?clamp(verifiedDataCount/totalDataCount*100):50;
 const metrics:HealthMetric[]=[
  ...
  {key:"compatibility",label:"Tür uyumu",score:compatibilityScore,status:status(compatibilityScore),detail:predationIssues.length?"Küçük canlılar için avlanma riski var":temperatureConflict||phConflict?"Su değeri aralıkları kesişmiyor":flowConflict?"Akıntı ihtiyaçları farklı":"Ortak yaşam aralıkları mevcut"},
 ...
 const missingCapacityCount=safetyEquipment.length-calculationReadyEquipment.length;
 ...
 if(missingCapacityCount)warnings.push({level:"warning",title:"Ekipman kapasite bilgisi eksik",message:`${missingCapacityCount} filtre veya ısıtıcının modeli doğrulandı; ancak güvenli kapasite hesabı için debi ya da hacim aralığı yayımlanmamış.`});
