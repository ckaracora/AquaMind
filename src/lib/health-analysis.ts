...
export function analyzeAquarium(aquarium:Aquarium,animals:Livestock[],equipment:Equipment[],latest?:WaterParameters):HealthAnalysis{
 ...
 const compatibilityPenalty=(temperatureConflict?40:0)+(phConflict?35:0)+(flowConflict?20:0)+(predationIssues.length?60:0)+(speciesOnlyIssues.length?60:0); const compatibilityScore=clamp(100-compatibilityPenalty);
 ...
 if(missingCapacityCount)warnings.push({level:"warning",title:"Ekipman kapasite bilgisi eksik",message:`${missingCapacityCount} filtre, hava motoru veya ısıtıcı kaydı güvenli kapasite hesabı için gerekli debi ya da hacim verisini sağlamıyor.`});
